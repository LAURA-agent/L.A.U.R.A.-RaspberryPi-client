...
# Notification tools
@mcp.tool()
async def check_notifications(session_id: str, ctx: Context = None) -> Dict[str, Any]:
    """Check for pending notifications for a session"""
    print(f"[SERVER] check_notifications called for session_id={session_id}")
    device_info = input_coordinator.get_device_info(session_id)
    if not device_info:
        raise ValueError("Invalid session_id for check_notifications")
    
    # Get notification manager from input coordinator
    notification_manager = getattr(input_coordinator, 'notification_manager', None)
    if not notification_manager:
        return {"notifications": [], "count": 0}
    
    result = await notification_manager.check_notifications_tool(session_id)
    await log_event(session_id, "notifications_checked", {"count": result.get("count", 0)})
    return result

@mcp.tool()
async def clear_reminder(session_id: str, reminder_type: str, ctx: Context = None) -> Dict[str, Any]:
    """Clear a specific reminder type"""
    print(f"[SERVER] clear_reminder called for session_id={session_id}, type={reminder_type}")
    device_info = input_coordinator.get_device_info(session_id)
    if not device_info:
        raise ValueError("Invalid session_id for clear_reminder")
    ...
    result = await notification_manager.clear_reminder_tool(reminder_type, session_id)
    await log_event(session_id, "reminder_cleared", {"type": reminder_type, "success": result.get("success", False)})
    return result
...
async def list_available_tools(ctx: Context = None) -> Dict[str, Any]:
    ...
    return {
        "tools": [
            ...
            {"name": "check_notifications", "description": "Check for pending notifications"},
            {"name": "clear_reminder", "description": "Clear a specific reminder"}
        ]
    }
# ==== SSE APP ONLY ====
app = mcp.sse_app()
